...
def td3(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    x_ph, a_ph, x2_ph, r_ph, d_ph, w_ph = core.placeholders(obs_dim, act_dim, obs_dim, None, None, None)
    ...
    q1_loss = tf.reduce_mean(w_ph * (q1 - backup) ** 2)
    q2_loss = tf.reduce_mean(w_ph * (q2 - backup) ** 2)
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            for j in range(ep_len):
                (obs1, act, done, rew, obs2), idxs, is_weight = replay_buffer.sample_batch(batch_size)
                feed_dict = {x_ph: obs1,
                             x2_ph: obs2,
                             a_ph: act,
                             r_ph: rew,
